...
<template>
  <div class="relative h-[70px] rounded-2xl overflow-clip drop-shadow">
    <img
      alt=""
    ...
    />
    <div
      class="absolute top-0 bottom-0 left-0 w-[70%]"
      style="background: linear-gradient(to right, #ffffffff 40%, #ffffff00)"
    >
      <div v-if="reGeoData.status == '1'" class="pt-3 pl-3 flex gap-2.5 items-center">
        <svg-icon name="Location" size="1.5em" />
        <div>
          <div class="text-sm font-bold">
            {{ reGeoData.regeocode.addressComponent.province }} {{ reGeoData.regeocode.addressComponent.city }}
